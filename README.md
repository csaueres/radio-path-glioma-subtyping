...
### Installation
Required packages are listed in TODO. A conda environment can be created by running the following command:
```
...
In this work we use GigaPath embeddings ([GigaPath](https://github.com/prov-gigapath/prov-gigapath)), but any should work (you may need to adjust the embedding size).

### Generate MRI Embeddings
Similarly, our pipeline expects MRI patch embeddings as .pth files. The format should be a dictionary with "features" containing a tensor of shape [n,p,e], describing number of samples, number of patches, and embedding size of each patch, respectively, and "id" mapping to the associated sample_ids in the same order. In our experiments we employ TODO
...
python train.py --lr 5e-5 --patch_frac 2000 --k 10 --seed 7 --max_epochs 30 --load_data_in_mem \
 --model_type <MODEL> --mri_embedder mm-dino --histo_embed_dim 1536 --exp_code run1 \
 ...
 MODEL can be any of: early-fusion_mamba, late-fusion3h_mamba, moe_mamba, histo_mamba, mri_mamba. We recommend moe_mamba. See common.py for additional possible models
...
python eval.py --checkpoint_dir /ckpts/moe-mamba_pretrained.pt --case_csv metadata/test_cases.csv \
     ...
     --mri_csv metadata/mri_test.csv --mri_root_dir data/mri --mri_embedder mm-dino \
```
